...
import { Button, Card, Notice, Row, Screen, Section, T } from '../src/ui/components/primitives';
...
export default function Backup() {
  ...
  const makeJson = () => serialiseBackup(createBackup(store.exportData(), { now: new Date(), appVersion: ENV.appVersion, catalogueVersion: CATALOGUE_VERSION }));

  const exportIt = async (toFolder: boolean) => {
    setBusy(true);
    setStatus(null);
    const name = backupFileName(today);
    const r = toFolder ? await saveBackupToFolder(makeJson(), name) : await shareBackup(makeJson(), name);
    if (r.ok) {
  ...
  const restore = async () => {
    ...
    setBusy(true);
    try {
      await store.restore(preview.data);
  ...
  return (
    <Screen>
      ...
      <Section title="Make a backup" subtitle={data.settings.lastBackupAt ? `Last backup: ${formatDay(data.settings.lastBackupAt.slice(0, 10), today)}` : 'No backup yet'}>
        <Card>
          <T variant="small">Includes your Garden Profile, areas, plantings and their history, journal, wish list, succession plans, reminder settings and task history. The plant catalogue and weather aren&apos;t included — the app recreates them.</T>
