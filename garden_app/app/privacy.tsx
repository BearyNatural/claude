...
const POINTS: { title: string; body: string }[] = [
  { title: 'Your garden is stored on this device', body: 'Your Garden Profile, garden areas, plantings, photos, journal, wish list, plans and settings are saved in the app\'s storage on this phone (or in this browser, for the web preview).' },
  ...
  { title: 'Your street address is optional and stays on this phone', body: 'Only if you use the optional garden map: the address you choose and the garden outlines you trace are saved on this device (and in backups you make), never on a server. Weather and climate keep using your rounded suburb location. You can remove the address and outlines at any time from Garden Profile.' },
  { title: 'What leaves your phone', body: 'When live weather is on, your approximate coordinates and timezone are sent to Open-Meteo (open-meteo.com) to get a forecast. If you use "Search more places online", your search text is sent to Open-Meteo\'s place search. If you use the optional garden map, the address you type is sent to an OpenStreetMap address search (Photon by Komoot, or Nominatim as a fallback) when you press Find address, and satellite map images for the area on screen are loaded from Esri. About once a day the app downloads the latest plant list from GitHub (plant data only — nothing is sent). If you look up a botanical name for a plant you add, that name is sent to the Atlas of Living Australia. These services receive the information needed to answer the request, like any website does (including your IP address). Nothing else is sent.' },
  { title: 'Plant photos stay on this phone', body: 'Photos you add to plants are shrunk and kept in the app\'s own storage on this device — not uploaded anywhere, and not added to your phone\'s gallery. They go into a backup only if you choose "Include photos" when backing up. Deleting a photo or a planting deletes its photo files.' },
